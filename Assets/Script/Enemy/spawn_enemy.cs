...
public class spawn_enemy : MonoBehaviour {
	...
	void Start () {
		string game_mode = PlayerPrefs.GetString("game_mode");

		if (game_mode.ToLower () == "survival")
						fishamt = 0;
		else if (game_mode.ToLower () == "scoring")
						fishamt = 20;
							...
							*/
							...
							while(colcnt < hitColliders.Length){
								if(hitColliders[colcnt].gameObject.tag == "Player"){
									...
									hazards[i].transform.position = Vector3.MoveTowards(hazards[i].transform.position, pt.position, step*-1);
									trig = true;
									Debug.Log("in range");
								}
								colcnt++;
							...
							if(!trig){
								hazards[i].transform.Translate( new Vector3(1*Random.Range(1f, 3f)*speed*Time.deltaTime/Random.Range(50f, 70f)
																			,ydir[i]*Time.deltaTime,0) );//cant multiple by Time.deltaTime on the x-axis 
							}
							else{
								trig = false;
							}
